...
- 图表 MCP：基于查询证据生成柱状图、折线图、饼图，支持 PNG / MinIO 链接、聊天内展示、历史回放和报告导出。
- TodoWrite 计划面板：复杂任务先规划、逐步更新状态，失败/取消保留未完成项。
- 原生 data-analysis Skill 按需加载，规定数据口径、查询验证、图表与证据报告工作流。
- 工具时间线：展示调用名称、脱敏参数、返回摘要、耗时和错误；服务端保存，刷新可恢复。
...
- 默认关闭，开启后按需连接；图表服务没启动不会阻止 Qiqi 启动或普通查数。调用失败显示图表错误并允许 Agent 继续解释数据。
...
保留 ReActAgent，启用原生 `reset_equipped_tools`：基础 SQL 工具常驻，`charts` / `files` / `web` 按需激活；web 组只在本次请求允许联网且搜索已配置时注册。
...
参数和结果通过 `EXECUTION_UPDATE` 规范化后展示；每条工具卡片可展开查看 SQL/参数、结果摘要及耗时。
...
刷新后恢复 Todo、工具、报告、证据和图表引用。若原运行仍在结束，页面短暂轮询状态；服务重启后的进行中记录标为未完成。
